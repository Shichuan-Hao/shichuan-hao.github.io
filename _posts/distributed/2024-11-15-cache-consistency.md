...
title: "高并发缓存一致性方案梳理"
description: "大部分面向公众的互联网系统，其并发请求数量与在线用户数量都是正相关的，而 MySQL 能够承担的并发读写量是有一定上限的，当系统的访问量超过一定程度的时候"
author: hsc
date: 2024-11-15 17:27:00 +0800
categories: [Java 后端, 分布式]
tags: ['分布式', '中间件', 'Redis', 'Kafka', 'RocketMQ']
mermaid: true
...
```mermaid
flowchart LRA[写请求] --> B[更新缓存] --> C[更新数据库]
```
...
```mermaid
flowchart LRA[写请求] --> B[更新数据库] --> C[更新缓存]
```
...
```mermaid
flowchart LRA[写请求] --> B[删除缓存] --> C[更新数据库]
```
...
```mermaid
sequenceDiagramparticipant B as 请求 B（查询）
    participant Cache as 缓存 participant DB as 数据库 participant A as 请求 A（更新）

    A->>Cache: 1. 删除缓存 B->>Cache: 2. 查询缓存，未命中 ❌B->>DB: 3. 读数据库，得到旧值 B->>Cache: 4. 将旧值写入缓存 A->>DB: 5. 更新数据库为新值
```
...
```mermaid
flowchart LRA[删除缓存] --> B[更新数据库] --> C[sleep N ms] --> D[再次删除缓存]
```
...
```java
// 延迟双删 —— 异步优化版 public void updateWithDelayDoubleDelete(String key, Object value) {redis.delete(key);                           // 1. 先删缓存 db.update(value);                            // 2. 更新数据库 asyncPool.execute(() -> {                     // 3. 异步延迟再删 Thread.sleep(500);                       // sleep 时间 ≈ 读 DB+写缓存耗时 redis.delete(key);
    });
}
```
...
```mermaid
flowchart TDsubgraph 读请求 A1[读缓存] -->|命中| A2[返回数据]A1 -->|未命中| A3[读数据库] --> A4[放入缓存] --> A2end

    subgraph 写请求 B1[更新数据库] --> B2[删除缓存]end
```
...
```mermaid
sequenceDiagramparticipant A as 请求 A（查询）
    participant Cache as 缓存 participant DB as 数据库 participant B as 请求 B（更新）

    Note over Cache: 缓存刚好失效 A->>DB: 1. 查询数据库，得到旧值 B->>DB: 2. 将新值写入数据库 B->>Cache: 3. 删除缓存 A->>Cache: 4. 将旧值写入缓存 → 脏数据！
```
...
```java
// 读 —— 先查缓存，未命中则查 DB 并回填 public Object read(String key) {Object value = redis.get(key);
    if (value != null) return value;            // 缓存命中，直接返回

    value = db.query(key);                      // 缓存未命中，查数据库 if (value != null) {redis.set(key, value, EXPIRE_TIME);     // 回填缓存 + 过期时间兜底}return value;
}

// 写 —— 先更新 DB，再删除缓存 public void write(String key, Object value) {db.update(key, value);                      // 1. 先更新数据库 redis.delete(key);                          // 2. 再删除缓存}
```
...
```mermaid
flowchart TDA[更新数据库成功] --> B[删除 Redis 缓存]B -->|删除失败 ❌| C[将 key 发送到消息队列]C --> D[消息队列投递消息]D --> E[消费端接收消息]E --> F[再次删除 Redis 缓存]B -->|删除成功 ✅| G[完成]
```
...
```mermaid
flowchart LRA[MySQL 更新] --> B[产生 binlog] --> C[Canal 采集] --> D[发送到 MQ] --> E[消费端删除缓存]
```
...
- **读**：先读缓存，未命中则读数据库并回填缓存- **写**：先更新数据库，再删除缓存
...
- 未命中缓存 → 直接更新数据库，返回- 命中缓存 → 更新缓存，再由 Cache 自己更新数据库（同步）
...
- 数据不是强一致性的- 可能丢数据（Unix/Linux 非正常关机会导致数据丢失）
