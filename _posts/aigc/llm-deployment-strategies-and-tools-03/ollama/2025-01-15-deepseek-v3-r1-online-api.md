...
一种简单的理解方法是: [Ollama REST API - OpenAI Compatibility](/posts/ollama-rest-api-OpenAI_Compatibility/) 是通过 Ollama 在本地部署了 DeepSeek R1 模型，最终的目的是能够提供类似 `http:localhost:11434/v1/chat/completions` 的接口，然后我们就可以像调用 `OpenAI` 的接口一样调用 `DeepSeek` 的接口了。这个过程需要我们本地的 `GPU` 资源，然后通过 `ollama` 来启动和管理模型。`DeepSeek` 的在线 API 接口，则不需要我们自己用本地的 `GPU` 资源去部署，而是由服务商部署好模型，然后我们通过注册账号，获取 `API Key`，然后就可以像调用 `OpenAI` 的接口一样调用 `DeepSeek` 的接口。
