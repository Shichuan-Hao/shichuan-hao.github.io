...
```java
public class OOMTest {public static List<Object> list = new ArrayList<>();

    // JVM 设置// -Xms10M -Xmx10M -XX:+PrintGCDetails -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=D:\jvm.dumppublic static void main(String[] args) {List<Object> list = new ArrayList<>();
        int i = 0;
        int j = 0;
        while (true) {list.add(new User(i++, UUID.randomUUID().toString()));
...
![jvisualvm 分析 dump]({{ '/assets/img/jvm/tuning-tools/page2_img2.png' | relative_url }})
...
```java
public class DeadLockTest {private static Object lock1 = new Object();
    private static Object lock2 = new Object();

    public static void main(String[] args) {new Thread(() -> {synchronized (lock1) {try {System.out.println("thread1 begin");
                    Thread.sleep(5000);
                } catch (InterruptedException e) {}synchronized (lock2) {System.out.println("thread1 end");
                }}}).start();

        new Thread(() -> {synchronized (lock2) {try {System.out.println("thread2 begin");
                    Thread.sleep(5000);
                } catch (InterruptedException e) {}synchronized (lock1) {System.out.println("thread2 end");
                }}}).start();
    }}
```
...
```
"Thread-1" 线程名 prio=5 优先级=5tid=0x000000001fa9e000 线程 idnid=0x2d64 线程对应的本地线程标识 nidjava.lang.Thread.State: BLOCKED 线程状态
```
...
public class Math {public static final int initData = 666;
    ...
    public static void main(String[] args) {Math math = new Math();
        while (true) {math.compute();  // CPU 飙高}}}
```
...
![jstack 高 CPU 线程]({{ '/assets/img/jvm/tuning-tools/page5_img1.png' | relative_url }})
...
```bash
jinfo -flags <pid>    # 查看 JVM 参数 jinfo -sysprops <pid> # 查看 Java 系统参数
```
...
![汇总 2]({{ '/assets/img/jvm/tuning-tools/page7_img1.png' | relative_url }})

![汇总 3]({{ '/assets/img/jvm/tuning-tools/page7_img2.png' | relative_url }})
...
```mermaid
graph TDA["jstat -gc pid 1000 10<br>(每隔 1 秒 1 次，共 10 次)"] --> B["观察 EU 增长 → 推算对象增长速率"]B --> C["根据 Eden 大小 → 推算 Young GC 频率"]C --> D["YGCT/YGC → 每次 Young GC 耗时"]D --> E["观察 S 区和 O 区变化 → 推算晋升对象量和老年代增长速率"]E --> F["FGCT/FGC → 推算 Full GC 频率和耗时"]
```
...
```mermaid
graph TDA[Full GC 比 Young GC 还多] --> B{排查方向}B --> C[元空间不够]B --> D[显示调用 System.gc]B --> E[老年代空间分配担保失败]C --> C1["增大 MetaspaceSize"]D --> D1["加 -XX:+DisableExplicitGC"]E --> E1["分析晋升情况"]
```
...
@RestControllerpublic class IndexController {
    ...
    public String processUserData() throws InterruptedException {ArrayList<User> users = queryUsers();
        for (User user : users) {System.out.println("user:" + user.toString());
        }return "end";
    }

    // 问题代码：一次查询 5000 个 User 对象！
    private ArrayList<User> queryUsers() {ArrayList<User> users = new ArrayList<>();
        for (int i = 0; i < 5000; i++) {users.add(new User(i, "zhuge"));
        }return users;
    }}
```
...
电商架构中使用多级缓存（Redis + JVM 级缓存），有些同学直接使用 HashMap 不断往里面放数据却没考虑容量限制，导致缓存 Map 越来越大一直占用老年代空间，时间长了就会导致频繁 Full GC 甚至 OOM。
...
```mermaid
graph LRA[调优工具链] --> B[jmap]A --> C[jstack]A --> D[jinfo]A --> E[jstat]A --> F[jvisualvm]B --> B1[内存分析/Dump]C --> C1[死锁/高 CPU 线程]D --> D1[JVM 参数查看]E --> E1[GC 统计/运行预估]F --> F1[远程监控/可视化]
```
