...
- **新生代**：每次收集都会有大量对象（近 99%）死去，可以选择**复制算法**，只需付出少量对象的复制成本即可完成每次垃圾收集。
...
```mermaid
graph LRA[垃圾收集算法] --> B[标记-复制]A --> C[标记-清除]A --> D[标记-整理]
    
    B --> B1["优点：速度快<br>缺点：浪费一半空间<br>适用：新生代"]C --> C1["优点：简单<br>缺点：碎片多、效率低<br>适用：老年代"]D --> D1["优点：无碎片<br>缺点：移动成本高<br>适用：老年代"]
```
...
```mermaid
graph TDsubgraph 新生代收集器 A1[Serial]A2[ParNew]A3[Parallel Scavenge]endsubgraph 老年代收集器 B1[Serial Old]B2[CMS]B3[Parallel Old]endsubgraph 全能收集器 C1[G1]C2[ZGC]C3[Shenandoah]endA1 -.-> B1A2 --> B2A3 --> B3
```
...
- 新生代采用**复制算法**，老年代采用**标记-整理算法**- 简单而高效，没有线程交互的开销，单线程收集效率高- Serial Old 收集器主要用于： JDK1.5 以前与 Parallel Scavenge 搭配，或作为 CMS 收集器的后备方案
...
- 新生代采用复制算法，老年代采用标记-整理算法- JDK8 **默认**的新生代和老年代收集器- 在注重吞吐量及 CPU 资源的场合优先考虑
...
- 新生代采用复制算法- 许多运行在 Server 模式下的虚拟机的首要选择- 除了 Serial 收集器外，**只有它能与 CMS 收集器配合工作**
...
sequenceDiagramparticipant GC as GC 线程 participant App as 用户线程
    ...
    App-->>App: 暂停 GC->>GC: 标记 GC Roots 直接引用对象
    
    Note over GC,App: 🟢 并发标记 App->>App: 正常运行 GC->>GC: 遍历整个对象图
    
    Note over GC,App: 🔴 重新标记（STW）
    App-->>App: 暂停 GC->>GC: 修正并发标记期间的变动
    
    Note over GC,App: 🟢 并发清理 App->>App: 正常运行 GC->>GC: 清理未标记区域
    
    Note over GC,App: 🟢 并发重置 GC->>GC: 重置标记数据
```
...
- 对象经过 5 次 Minor GC 才进入老年代，整个时间约一两分钟- 如果对象这么长时间都没被回收，可以认为这些对象会长久存活
...
但压测场景下，某次 Minor GC 后可能有超过一两百 M 存活对象直接进入老年代。估算**大概每隔五六分钟**出现一次这样的情况，大约**半小时到一小时**之间会因老年代满触发一次 Full GC。
...
```mermaid
graph TDA[漏标问题] --> B[增量更新 CMS]A --> C[SATB G1/Shenandoah]B --> B1["插入引用时记录<br>黑色→灰色<br>重新深度扫描"]C --> C1["删除引用时记录<br>简单标记为黑<br>下一轮再扫描"]B --- B2["效率较低<br>浮动垃圾少"]C --- C2["效率较高<br>浮动垃圾多"]
```
...
```java
void oop_field_store(oop* field, oop new_value) {pre_write_barrier(field);         // 写屏障 - 写前操作*field = new_value;
...
```java
void pre_load_barrier(oop* field) {oop old_value = *field;
...
- 使用一个**字节数组**实现：`CARD_TABLE[]`- 每个元素对应标识的内存区域一块特定大小的内存块，称为**"卡页"**- HotSpot 使用的卡页大小是 **2^9 = 512 字节**

![卡表结构]({{ '/assets/img/jvm/parnew-cms/page11_img1.jpeg' | relative_url }})

```mermaid
graph LRsubgraph 堆内存 A[卡页 0 512B] --- B[卡页 1 512B] --- C[卡页 2 512B] --- D[...]endsubgraph 卡表数组 T0[0] --- T1[1] --- T2[0] --- T3[...]endB -->|"有跨代引用 → 变脏"| T1
```
...
```mermaid
graph TDA[垃圾收集算法] --> B[分代收集]B --> C[新生代: 复制算法]B --> D[老年代: 标记-清除/标记-整理]C --> E[ParNew 收集器]D --> F[CMS 收集器]E --> G[亿级电商系统调优]F --> GG --> H[三色标记算法]H --> I[增量更新 CMS]H --> J[SATB G1/Shenandoah]H --> K[读屏障 ZGC]G --> L[记忆集与卡表]
```
