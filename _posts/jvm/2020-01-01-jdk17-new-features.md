...
![JDK17 新特性概览]({{ site.url }}/assets/img/jvm/jdk17-features/page1_img1.png)
...
System.out.println(p.x() + "====" + p.y());  // 10====20// 不允许通过反射修改值
```
...
```java
// final: 不能再被继承 public final class Circle extends Shape { ... }

// non-sealed: 可以随意继承 public non-sealed class Square extends Shape { ... }

// sealed: 继续密封，声明自己的子类 public sealed class Rectangle extends Shape permits FilledRectangle { ... }public final class FilledRectangle extends Rectangle { ... }
```
...
```java
module roy.demomodule {requires junit;       // 第三方依赖 requires java.sql;    // JDK 内置模块}
```
...
```java
module roy.demomodule {exports com.roy.language;   // 对外暴露包（编译+运行时可用，不可反射）
...
}// 输出： good morning loulan//       good evening loulan
```
...
```bash
# 指定模块路径运行
java --module-path demoModule/demoModule.jar:demoModule2/demoModule2.jar \-m roy.demomodule/com.roy.spi.ServiceDemo
...
- 启动速度慢- 预热时间长才能达到最佳性能- 不适合短生命周期服务
...
```bash
# 下载 tar 包后解压使用
tar -xzf graalvm-jdk-17.0.9-linux-aarch64.tar.gzexport JAVA_HOME=/path/to/graalvmexport PATH=$JAVA_HOME/bin:$PATHjava -version
