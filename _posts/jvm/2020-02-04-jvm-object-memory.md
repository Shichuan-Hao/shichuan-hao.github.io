...
description: "一个 在 JVM 内部到底经历了什么？对象的内存怎么分配、何时回收？这些看似基础的问题，恰恰是理解 JVM 运作机制的关键入口。"
author: hsc
...
```xml
<dependency><groupId>org.openjdk.jol</groupId><spanrtifactId>jol-core</artifactId><version>0.9</version></dependency>
```
...
public class JOLSample {
    ...
    public static void main(String[] args) {// 查看普通 Object 的大小 ClassLayout layout = ClassLayout.parseInstance(new Object());
        System.out.println(layout.toPrintable());

        System.out.println();

        // 查看空数组的大小 ClassLayout layout1 = ClassLayout.parseInstance(new int[]{});
        System.out.println(layout1.toPrintable());

        System.out.println();

        // 查看自定义对象的大小 ClassLayout layout2 = ClassLayout.parseInstance(new A());
        System.out.println(layout2.toPrintable());
    }

    // -XX:+UseCompressedOops           默认开启，压缩所有指针// -XX:+UseCompressedClassPointers  默认开启，压缩对象头里的 Klass Pointer// Oops : Ordinary Object Pointerspublic static class A {// 8B  mark word// 4B  Klass Pointer（关闭压缩则 8B）
        int id;        // 4BString name;   // 4B（关闭压缩则 8B）
        byte b;        // 1BObject o;      // 4B（关闭压缩则 8B）
    }}
```
...
```java
4          (object header)                01 00 00 00  // MarkWord 前半 4          (object header)                00 00 00 00  // MarkWord 后半 4          (object header)                e5 01 00 f8  // Klass Pointer4          (loss due to alignment)
```
Instance size: 16 bytes                        ← Object 占用 16 字节

[A 对象]OFFSET  SIZE   TYPE   DESCRIPTION                    VALUE
```java
4          (object header)                01 00 00 004          (object header)                00 00 00 004          (object header)                61 cc 00 f84    int   A.id                           01    byte  A.b                            03          (alignment/padding gap)
    (object header)                01 00 00 004          (object header)                00 00 00 004          (object header)                61 cc 00 f84    int   A.id                           01    byte  A.b                            03          (alignment/padding gap)
    String A.name                        null4    Object A.o                           null4          (loss due to alignment)

4    String A.name                        null4    Object A.o                           null4          (loss due to alignment)
```
Instance size: 32 bytesSpace losses: 3 bytes internal + 4 bytes external = 7 bytes total
```
...
- 主内存和缓存之间移动数据的**带宽占用更大**- GC 承受的**压力更大**

**压缩原理**：

- JDK 1.6 update14 开始，64 位 JVM 支持指针压缩- 配置参数：`-XX:+UseCompressedOops`（默认开启）
- 32 位地址最大支持 4G 内存（2³²），通过压缩编码/解码优化，使得 32 位地址支持到 **32G** 以内- 堆内存 **< 4G** 时，直接去除高 32 位地址，无需压缩- 堆内存 **> 32G** 时，压缩指针**失效**，强制使用 64 位寻址 → 内存膨胀、 GC 压力增大
...
```java
// 发生逃逸——user 对象被返回，作用域不确定 public User test1() {User user = new User();
    user.setId(1);
    user.setName("zhuge");
    return user;
}

// 未逃逸——user 对象在方法结束后即为无效，可栈上分配 public void test2() {User user = new User();
    user.setId(1);
    user.setName("zhuge");
}
```
...
```java
/*** 栈上分配 + 标量替换 验证* -Xmx15m -Xms15m -XX:+DoEscapeAnalysis -XX:+PrintGC -XX:+EliminateAllocations*/public class AllotOnStack {
...
```mermaid
graph LRsubgraph 新生代 Eden["Eden (80%)"]S0["S0 (10%)"]S1["S1 (10%)"]endOld["老年代"]
    
    Eden -->|"Minor GC → 存活→"| S0S0 -.->|"下次 Minor GC → 存活→"| S1S1 -.->|"下次 Minor GC → 存活→"| S0S0 -->|"年龄 ≥ 15"| OldS1 -->|"年龄 ≥ 15"| Old
```
...
```java
// JVM 参数：-XX:+PrintGCDetailspublic class GCTest {public static void main(String[] args) {byte[] allocation1, allocation2;

        allocation1 = new byte[60000 * 1024];  // 约 60M

        // 此时再分配 8M，Eden 不够 → 触发 Minor GC// allocation1(60M) 太大无法放入 Survivor → 直接进老年代 allocation2 = new byte[8000 * 1024];
    }}
```
...
PSYoungGen   total 76288K, used 9591Keden  space 65536K, 13% usedfrom  space 10752K, 8% usedto    space 10752K, 0% usedParOldGen    total 175104K, used 60008Kobject space 175104K, 34% used
```

关键信息：
- Eden 区 100% → Minor GC 后变为 13%- `allocation1`（60M）因为太大无法放入 Survivor（仅 10M），被**直接晋升到老年代**- 老年代 used 从 0 变为 60008K（正好约 60M），验证了晋升行为- allocation2（8M）GC 后正常分配到 Eden 区
...
```mermaid
graph LRA["1. 对象创建<br/>5 个步骤"] --> B["2. 内存分配<br/>3 条路径"]B --> C["3. 内存回收<br/>可达性分析"]
    
    A1["类加载检查 → 分配内存 → 零值初始化 → 对象头设置 → init"] -.-> AB1["栈上分配（逃逸分析+标量替换）<br/>→ Eden 分配 → 老年代分配"] -.-> BC1["GC Roots 搜索 → 引用类型判断 → finalize 自救"] -.-> C
```
