...
description: "理解了 JVM 的整体架构和类加载机制之后，下一步自然就是深入 JVM 的内存模型。"
author: hsc
date: 2020-02-15 14:30:00 +0800
...
![JDK 体系结构](/assets/img/jvm/memory-model/page1_img1.png)
...
![Java 跨平台特性](/assets/img/jvm/memory-model/page1_img2.png)
...
![JVM 整体结构及内存模型](/assets/img/jvm/memory-model/page2_img1.png)
...
```mermaid
graph TBsubgraph 堆内存 Heapsubgraph 新生代 YoungGen["新生代 Young Generation (1/3)"]Eden["Eden 区 (8/10)"]S0["Survivor 0 (1/10)"]S1["Survivor 1 (1/10)"]endOld["老年代 Old Generation (2/3)"]end
    
    Eden -->|"Minor GC → 存活对象"| S0S0 -->|"下次 Minor GC → 存活对象"| S1S1 -->|"每 GC 一次，分代年龄 +1"| S0S0 -->|"年龄达到阈值(默认 15)"| OldS1 -->|"年龄达到阈值(默认 15)"| Old
```
...
![JVM 内存参数设置](/assets/img/jvm/memory-model/page2_img2.png)
...
- **`-XX:MetaspaceSize`**：指定元空间触发 Full GC 的初始阈值，默认是 **21M**。当元空间使用量达到该值时会触发 Full GC 进行类型卸载，同时 GC 收集器会对该值进行动态调整：
  - 释放了大量空间 → 适当**降低**该值- 释放了很少空间 → 在不超 `MaxMetaspaceSize` 的前提下适当**提高**该值
...
public class StackOverflowTest {
    ...
    public static void main(String[] args) {try {redo();
...
```
java.lang.StackOverflowErrorat com.tuling.jvm.StackOverflowTest.redo(StackOverflowTest.java:12)
...
![百万级订单系统 JVM 参数](/assets/img/jvm/memory-model/page4_img1.png)
...
```
java -Xms4096M -Xmx4096M -Xmn2048M -Xss512K \-XX:MetaspaceSize=256M -XX:MaxMetaspaceSize=256M \-XX:+PrintGCDetails -XX:+PrintGCDateStamps \-Xloggc:/path/to/gc.log \-jar order-service.jar
```
